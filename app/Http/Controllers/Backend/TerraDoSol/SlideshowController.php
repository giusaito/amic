...
class SlideshowController extends Controller
{
    ...
    public function index(Edition $edicao){
        $records = Slideshow::where(['ts_edition_id'=> $edicao->id])->orderBy('id', 'desc')->get();
        return view('Backend.TerraDoSol.Slideshow.index', compact('edicao','records'));
    }
    ...
    {
        $record = Slideshow::find($request->key);
        if($record->delete()){
