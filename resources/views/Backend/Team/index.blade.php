            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="ibox-title">
                        <h5>Lista de equipe cadastrada</h5>
                        <div class="ibox-tools">
                            <a href="{{route('backend.equipe.create')}}/" class="btn btn-primary btn-sm right">
                    ...
                    <hr>
                    <form action="{{route('backend.equipe.search')}}" method="GET">
                       <div class="col-12">
                        ...
                        <tbody>
                            @foreach($records as $record)
                                <tr>
                                    ...
                                    <th scope="row">{{$record->office}}</th>
                                    {{-- <th scope="row">{{ Str::limit($record->description, 50) }}</th> --}}
                                    <th scope="row">
                                        <a href="{{route('backend.equipe.edit', ['equipe' => $record->id])}}" class="btn btn-warning btn-sm">
                                            <i class="fa fa-pencil"></i> 
                                            Editar
                                        </a>
                                        <form  method="POST" action="{{route('backend.equipe.destroy', ['equipe' => $record->id])}}" style="display:inline-block">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
