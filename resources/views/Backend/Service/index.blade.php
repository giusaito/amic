            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="ibox-title">
                        <h5>Lista de Serviços cadastrados</h5>
                        <div class="ibox-tools">
                            <a href="{{route('backend.servico.create')}}/" class="btn btn-primary btn-sm right">
                    ...
                    <hr>
                    <form action="{{route('backend.servico.search')}}" method="GET">
                       <div class="col-12">
                        ...
                        <tbody>
                            @foreach($records as $record)
                                <tr>
                                    ...
                                    <th scope="row">{{$record->email_to}}</th>
                                    <th scope="row">
                                        <a href="{{route('backend.servico.edit', ['servico' => $record->id])}}" class="btn btn-warning btn-sm">
                                            <i class="fa fa-pencil"></i> 
                                            Editar
                                        </a>
                                        <form  method="POST" action="{{route('backend.servico.destroy', ['servico' => $record->id])}}" style="display:inline-block">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
